...
def run_prediction(data_path, pt_ckpt_dir_path, save_path, tag, debug=False):  
    ...
    print(ckpt_name)
    state_dict = torch.load(pt_ckpt_path, map_location=device, weights_only=False)['model_state_dict']###state_dict = torch.load(pt_ckpt_path, map_location=device)['model_state_dict']
